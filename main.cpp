...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  while (Novice::ProcessMessage() == 0) {
    ...
    Matrix4x4 cameraMatrix = MakeAffineMatrix(
        {1.0f, 1.0f, 1.0f}, {0.0f, 0.0f, 0.0f}, cameraTranslate);
    ...
    // 最初にスクリーン座標を計算
    for (int i = 0; i < 3; ++i) {
      Vector4 clip = Transform4(kLocalVertices[i],
                                worldViewProjectionMatrix); // W込みで変換
      Vector3 ndc = ToNDC(clip);                            // Wで割って正規化
      screenVertices[i] = Transform(ndc, viewportMatrix);   // ビューポート変換
    }
    ...
    Novice::DrawTriangle(static_cast<int>(screenVertices[0].x),
                         ...
                         static_cast<int>(screenVertices[2].y), RED,
                         kFillModeSolid);
